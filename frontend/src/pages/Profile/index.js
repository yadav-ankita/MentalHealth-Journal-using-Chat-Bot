...
export default function Profile() {
  const { user, allSymptom, getAllChat, allChats, getAllSymptom, userInfo, getProfileInfo, getHealthGoals, creteHealthGoal, healthGoal, isLoading } = useGlobalContext();
  useEffect(() => {
    getAllChat();
    getProfileInfo();
    getAllSymptom();
    getHealthGoals();
  },[getAllChat, getAllSymptom, getHealthGoals, getProfileInfo])
  const today = new Date().toISOString().slice(0, 10);
  const [description, setDescription] = useState('');
  const [targetDate, setTargetdate] = useState(today);
  const handleHealth = async () => {
    await creteHealthGoal({ description, targetDate });
    getHealthGoals();
    setDescription('');
    setTargetdate(today);
  }
              ...
              <div className='add-health profile-info'>
                ...
                <div className='health-btn'>
                  <div className='profile-wrap profile-header' style={{ backgroundColor: '#0118D8', margin: 'auto', width: '100%' }}>
                    ...
                    <button style={{ backgroundColor: '#0118D8', border: 'none', color: 'white', cursor: 'pointer' }} onClick={handleHealth}>Add New Health Goals</button>
                  </div>
                </div>
              </div>
